...
export interface LogTransport {
  enabled: boolean;
  log(level: string, formattedMessage: string, args: unknown[]): void;
}
...
export class ConsoleTransport implements LogTransport {
  ...
  log(level: string, formattedMessage: string, args: unknown[]) {
    if (!this.enabled) {
      return;
    }
...
export class FileTransport implements LogTransport {
  ...
  log(level: string, formattedMessage: string, args: unknown[]) {
    ...
    try {
      fs.appendFileSync(
        this.logFile,
        `[${level}] ${formattedMessage} ${args.map((arg) => JSON.stringify(arg)).join(' ')}\n`,
      );
    } catch (err) {
