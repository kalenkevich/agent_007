...
import {type ChatMessage} from './chat/chat_message.js';
import {processEvent, type ChatState} from './chat/event_processor';
...
export default function App() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'init',
      content:
        'Welcome to Agent 007 Neural Interface. Awaiting your instructions...',
      isUser: false,
    },
  ]);
  ...
  const chatStateRef = useRef<ChatState>({
    ...
    isThinking,
    pendingUserInput,
    activeStreamMessageId: activeStreamMessageIdRef.current,
  });
  ...
  useEffect(() => {
    ...
    agentClient.onAgentEvent((event: AgentEvent) => {
      ...
      activeStreamMessageIdRef.current = newState.activeStreamMessageId;
    });
  ...
  const appendMessage = (content: string, isUser: boolean) => {
    setMessages((prev) => [
      ...prev,
      {id: crypto.randomUUID(), content, isUser},
    ]);
  };
        ...
        <main className="chat-area">
          ...
          <div
            ...
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`message ${msg.isUser ? 'user-msg' : 'system-msg'}`}>
