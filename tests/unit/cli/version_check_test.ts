...
describe("version_check", () => {
  ...
  it("should do nothing if version is up to date", async () => {
    vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ version: "0.0.2" }));
  ...
  it("should do nothing if current version is newer (dev)", async () => {
    vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ version: "0.0.3" }));
  ...
  it("should prompt and attempt update if version is stale and user says yes", async () => {
    ...
    expect(child_process.spawnSync).toHaveBeenCalledWith("npm", ["install", "-g", "@kalenkevich/agent_007"], expect.any(Object));
    expect(exitSpy).toHaveBeenCalledWith(0);
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Update successful!"));
  });
  ...
  it("should handle update failure and show manual command", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Update failed."));
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Please try running manually"));
  });
